...
import { Package, Plus, Search, Loader2, Boxes, ArrowRightLeft, Trash2, Eye, Warehouse, Barcode, Link2, FileSpreadsheet, AlertTriangle, Smartphone, Edit } from 'lucide-react';
...
const MOVEMENT_META: Record<string, { label: string; cls: string; sign: '+' | '-' | null }> = {
  ...
  ajustement: { label: 'Ajustement', cls: 'bg-[#1a2420] text-[#7a8f80] border-[#1e2e25]', sign: null },
  ...
  const focusMut = useMutation((file: File) => stockService.focusImport(file), {
    onSuccess: (r: any) => {
      toast({
        ...
        description: `${r?.createdItems ?? 0} article(s), ${r?.updatedLevels ?? 0} niveau(x)`,
        variant: 'success',
      });
      refetch();
    },
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="secondary" onClick={() => setShowMovement(true)}><ArrowRightLeft size={15} /> Enregistrer mouvement</Button>
          {tab !== 'BORDEREAU' && tab !== 'PARC' && tab !== 'OUTILLAGE' && <Button onClick={() => setShowCreate(true)}><Plus size={15} /> Ajouter article</Button>}
        </div>
      </div>
        ...
        <>
                      ...
                      <tr key={item.id} className="hover:bg-[#172019] transition-colors cursor-pointer" onClick={() => setDetailItem(item)}>
                        ...
                        <td className="px-4 py-3 text-right" onClick={e => e.stopPropagation()}>
                          <div className="flex items-center justify-end gap-1">
                            ...
                            <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-[#7a8f80] hover:text-[#0f9d70]" onClick={() => setEditItem(item)} title="Modifier"><Edit size={14} /></Button>
                            <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-[#7a8f80] hover:text-[#C0392B]" onClick={() => setDeleteId(item.id)} title="Supprimer"><Trash2 size={14} /></Button>
                          </div>
                        </td>
                      </tr>
                    );
          ...
          <Card className="border-[#1e2e25] bg-[#111916] overflow-hidden">
            <div className="px-4 py-3 border-b border-[#1e2e25] flex items-center justify-between">
              ...
              <Button size="sm" variant="secondary" onClick={() => setShowMovement(true)}><Plus size={13} /> Mouvement</Button>
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-[#1e2e25]/50 bg-[#111916]">
                  {mvList.slice(0, 20).map((mv: any) => {
                    ...
                    return (
                      <tr key={mv.id} className="hover:bg-[#172019] transition-colors">
                  ...
                  {mvList.length === 0 && (
                    <tr><td colSpan={7} className="px-4 py-8 text-center text-sm text-[#7a8f80]">Aucun mouvement enregistré</td></tr>
                  )}
                </tbody>
              </table>
            </div>
      ...
      {/* Mouvement */}
      <MovementModal open={showMovement} onClose={() => setShowMovement(false)} items={itemsList} warehouses={whList} onDone={() => { refetch(); refetchMv(); }} />

      {/* Fiche article */}
      <ItemDetailModal item={detailItem} onClose={() => setDetailItem(null)} levels={detailItem ? (levelsByItem[detailItem.id] ?? []) : []} />

      <ConfirmDialog open={!!deleteId} onClose={() => setDeleteId(null)} title="Supprimer l'article"
        message="Cette action est irréversible — les niveaux et mouvements associés seront supprimés."
...
function BordereauTab() {
  ...
  const { data, loading } = useQuery(
    () => {
      ...
      if (search) return api.get(`/price-items/search?q=${encodeURIComponent(search)}`);
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <span className="text-xs text-[#7a8f80] ml-auto flex items-center gap-1"><FileSpreadsheet size={13} /> Bordereau 3STB 2025 — {items.length} item(s)</span>
      </div>
      {loading ? (
...
function MovementModal({ open, onClose, items, warehouses, onDone }: {
  open: boolean; onClose: () => void; items: any[]; warehouses: any[]; onDone: () => void;
}) {
  const { toast } = useToast();
  const [f, setF] = useState<Record<string, any>>({ type: 'entree', stockItemId: '', quantity: 1, fromWarehouseId: '', toWarehouseId: '', note: '' });

  useEffect(() => { if (open) setF({ type: 'entree', stockItemId: '', quantity: 1, fromWarehouseId: '', toWarehouseId: '', note: '' }); }, [open]);

  const mut = useMutation((d: any) => stockService.createMovement(d), {
    onSuccess: () => { toast({ title: 'Mouvement enregistré', variant: 'success' }); onClose(); onDone(); },
    onError: (e: any) => toast({ title: 'Mouvement refusé', description: e.message, variant: 'error' }),
  });

  const needsFrom = ['transfert', 'consommation', 'echange_sav', 'ajustement', 'retour'].includes(f.type);
  const needsTo = ['entree', 'transfert'].includes(f.type);

  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    const payload: any = { type: f.type, stockItemId: f.stockItemId, quantity: Number(f.quantity), note: f.note || undefined };
    if (needsFrom && f.fromWarehouseId) payload.fromWarehouseId = f.fromWarehouseId;
    if (needsTo && f.toWarehouseId) payload.toWarehouseId = f.toWarehouseId;
    mut.mutate(payload);
  };
  ...
  return (
    <Modal open={open} onClose={onClose} title="Enregistrer un mouvement">
      <form className="space-y-3" onSubmit={submit}>
        ...
        <Select label="Article *" value={f.stockItemId} onChange={e => setF({ ...f, stockItemId: e.target.value })} required>
          <option value="">— Sélectionner —</option>
          {items.map(i => <option key={i.id} value={i.id}>{i.reference} — {i.designation}</option>)}
        </Select>
        <Input label="Quantité *" type="number" min={1} value={f.quantity} onChange={e => setF({ ...f, quantity: e.target.value })} required />
        <div className="grid grid-cols-2 gap-3">
          {needsFrom && (
            <Select label="Dépôt source" value={f.fromWarehouseId} onChange={e => setF({ ...f, fromWarehouseId: e.target.value })}>
              <option value="">—</option>
              {warehouses.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}
            </Select>
          )}
          {needsTo && (
            <Select label="Dépôt destination" value={f.toWarehouseId} onChange={e => setF({ ...f, toWarehouseId: e.target.value })}>
              <option value="">—</option>
              {warehouses.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}
            </Select>
          )}
        ...
        <div className="flex gap-2 justify-end pt-1">
          <Button type="button" variant="secondary" onClick={onClose}>Annuler</Button>
          <Button type="submit" disabled={mut.loading || !f.stockItemId}>{mut.loading && <Loader2 size={14} className="animate-spin" />} Enregistrer</Button>
        </div>
      </form>
    </Modal>
  );
...
/* ═════════════════ FICHE ARTICLE ═════════════════ */
function ItemDetailModal({ item, onClose, levels }: { item: any | null; onClose: () => void; levels: any[] }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const s = await api.get(`/stock-items/${item.id}/serials`);
  ...
  const loadTrace = async (serial: any) => {
    try {
      const ev = await api.get(`/stock-serials/${serial.id}/traceability`);
      setTrace({ serial, events: Array.isArray(ev) ? ev : [] });
    } catch {
                ...
                <div key={ev.id ?? i} className="flex items-start gap-3">
                  ...
                  <div className="pb-3">
                    ...
                    <p className="text-xs text-[#e8ede9] mt-1">
                      {ev.fromWarehouse?.name ?? 'Entrée'} → {ev.toWarehouse?.name ?? 'Consommé'}
                    </p>
